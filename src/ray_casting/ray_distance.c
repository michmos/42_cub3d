...
const double	get_vec_len(t_vec start, t_vec end, t_deg angle)
{
	double	cath1;
	double	cath2;
	double	hypoth;
	...
	if ((uint32_t)angle % 90 < 5 || (uint32_t)angle % 90 > 85)
	{
	...
	if ((angle > 90 && angle < 180) || angle > 270)
		alpha = 90 - ((u_int16_t)(angle) % 90 + (angle - (uint16_t)angle));
	else
		alpha = (u_int16_t)(angle) % 90 + (angle - (uint16_t)angle);
	return (cath1 / cos(deg_to_rad(alpha)));
	//= abs(1/start.y-end.y)
	//= abs(1/start.x-end.y)
}
...
void	find_distance_to_wall(t_ray	*ray, t_cor_px pov, t_deg view_angle, t_map *map)
{
	...
	if (ray->angle != 0 && ray->angle != 180)
	{
		...
		hor_hit_distance = get_vec_len(pov, hor_intersec_hit, ray->angle);
	}
	...
	if (ray->angle != 90 && ray->angle != 270)
	{
		...
		ver_hit_distance = get_vec_len(pov, ver_intersec_hit, ray->angle);
	}
