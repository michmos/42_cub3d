...
static void	put_scaled_wall(mlx_image_t *nxt_frame, t_wall_data *wall_info,
		u_int16_t frame_x_pos, u_int16_t frame_y_pos)
{
	double		scale_factor;
	u_int16_t	wall_y;
	...
	{
		pxl_rgba = get_pxl_rgba(wall_info->img, (scld_txtre_y_start + wall_y)
				/ scale_factor, wall_info->txtre_x_pos);
...
static void	put_ceiling(mlx_image_t *img, size_t upper_wall_end, size_t column,
		int rgb)
{
	size_t	y;
...
static void	put_floor(mlx_image_t *img, size_t lower_wall_end, size_t column,
		int rgb)
{
	size_t	y;
...
static t_wall_data	get_wall_data(t_walls *wall_imgs, t_ray *ray,
		u_int16_t scaled_height, u_int16_t center)
{
		...
		u_int16_t scaled_height, t_ray *ray)
{
	...
	put_ceiling(cub3d->nxt_frame, wall_data.upper_end, frame_x_pos,
		cub3d->ceiling.rgb);
	put_scaled_wall(cub3d->nxt_frame, &wall_data, frame_x_pos,
		wall_data.upper_end);
	put_floor(cub3d->nxt_frame, wall_data.upper_end + scaled_height,
		frame_x_pos, cub3d->floor.rgb);
}
